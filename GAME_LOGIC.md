...
### `gameMode`
- **'pvp'**: Игрок против игрока (онлайн режим - оба игрока играют одновременно)
...
- Могут быть уничтожены башнями и другими солдатами. При уничтожении - исчезают. в окошке статистики перемещаются в категорию убитые.
...
#### Генерация
- Золото генерируется случайно на карте при старте игры только на свободных ячейках.
...
- регенерация через настраиваемый временной промежуток. 

#### Механика
- Золото собирается сборщиками
- Каждая куча имеет определённое количество золота
- Золото уменьшается по мере сбора
- Когда куча опустошена, она помечается как собранная. исчезает при следующей регенерации золота.
...
2. **Обновление солдат** (`updateSoldiers`) - только если `gameState === 'playing'`
   ...
   - Проверка зависания (если солдат не двигается > 10 секунд)
   - Пересчёт пути при зависании
...
3. **Обновление рабочих** (`updateWorkers`) - только если `gameState === 'playing'`
   ...
   - Возврат на базу
   - Проверка зависания (если рабочий не двигается > 10 секунд)
   - Пересчёт пути при зависании
...
#### Онлайн режим (PvP)
**Особенности:**
- Оба игрока играют одновременно
- Нет поочерёдных ходов
- Каждый игрок может размещать объекты на своей территории в любое время
- Действия выполняются в реальном времени
...
## Проверка зависания (Stuck Detection)

### Механика проверки

Система автоматически отслеживает зависшие объекты (солдаты и рабочие), которые перестали двигаться.

#### Критерии зависания

**Для солдат:**
- Солдат не двигается более 10 секунд
- Путь не найден и солдат не рядом с базой
- Солдат застрял в одной позиции без движения

**Для рабочих:**
- Рабочий не двигается более 10 секунд
- Путь не найден и рабочий не выполняет задачу (сбор/строительство)
- Рабочий застрял в одной позиции без движения

#### Обработка зависания

**Автоматические действия:**
1. **Пересчёт пути**: Если объект завис, система пытается пересчитать путь от текущей позиции
2. **Удаление объекта**: Если путь не найден и объект не может двигаться более 10 секунд, объект удаляется
3. **Логирование**: Все случаи зависания логируются для отладки

#### Периодическая проверка

**Интервалы проверки:**
- Проверка движения: каждые 1 секунду
- Пересчёт пути: каждые 5 секунд (если объект не двигается)
- Удаление зависших объектов: после 10 секунд без движения

#### Предотвращение зависания

**Меры предосторожности:**
- Периодический пересчёт пути для учёта изменений препятствий
- Проверка блокировки следующего шага пути перед движением
- Автоматическое удаление объектов без пути (кроме тех, что рядом с базой)
- Очистка золота с баз каждые 5 секунд (чтобы не блокировать ворота)

#### Логирование зависания

**Отладочная информация:**
- Время последнего движения объекта
- Текущая позиция и целевая позиция
- Состояние пути (есть/нет, длина)
- Причина зависания (путь не найден, заблокирован, и т.д.)
